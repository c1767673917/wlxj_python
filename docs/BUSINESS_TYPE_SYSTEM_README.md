...
已成功将现有的用户数据隔离模式改为业务类型数据共享模式，支持`oil`(油脂)、`fast_moving`(快消)两种业务类型和`admin`(管理员)角色。
...
### 测试账户
- `oil_user1` / `password123` - 油脂
- `oil_user2` / `password123` - 油脂  
- `fast_user1` / `password123` - 快消
...
✅ **同业务类型数据共享**：油脂用户可以看到彼此的供应商和订单
✅ **业务类型数据隔离**：油脂和快消数据完全隔离
